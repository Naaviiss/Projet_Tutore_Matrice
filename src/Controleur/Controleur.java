...
public class Controleur implements ActionListener,MouseListener{
	...
	public Controleur(PanelChoix pPanChoix) {
		...
		List<String> chLigneModif = new ArrayList<String>();//liste des opérations effectuées sous forme de chaine de caractéres
	...
	public void actionPerformed(ActionEvent pEvt){
		...
		if(pEvt.getActionCommand().equals(Data.VALIDER_DIDACTICIEL)){
			...
			Matrice actuelle;//on récupére la matrice sur laquelle on travaille
			...
			for (int i = 0;i<=6;i++) {
				if (i == 4) {
					...
					actuelleID = chPanAffichageMatrices.getChMatricesID().get(chPanAffichageMatrices.getChMatricesID().size()-1);//idem pour son identité
			...
			panCom = chPanGauss.getPanelCommandes();//on récupére le panel commande		
		}
		...
		if(Arrays.asList(Data.FLECHES).contains(pEvt.getActionCommand())) { //si la commande de la source est une fléche
		...
		if(pEvt.getActionCommand().equals(Data.VALIDER_PANEL_COMMANDES)) {
			...
			int ligneModifiee = getNumLigne(operation[0]); //on récupére la ligne à modifier
		...
		if(pEvt.getActionCommand().equals(Data.CONSTANTE)) { //si la commande de la source est le bouton constante
			if( operation[1].equals(Data.FLECHES[0]) && (operation[2].equals("") || Arrays.asList(Data.LIGNES).contains(operation[2])) ) { //on  ne peut avoir une constante qu'avec la flèche <- et si une constante n'a pas déjÃ  été entrée auparavant
				...
				try {
					...
					if (labelVideConstante == 2) {
						//la ligne qui suit doit être la même que celle Ã  modifier
	...
	public void mouseClicked(MouseEvent e) {//si on clique sur une ligne
		...
		if ( labelVide == 0 || operation[1].equals(Data.FLECHES[0]) ){ //si on choisit la ligne Ã  modifier ou si on a utilisé la flèche <-
			if (labelVide == 0) {
			...
			if( labelVide == 2 && operation[0].equals( e.getComponent().getName() ) ) {//si la première ligne après la flèche est la mÃªme que celle Ã  modifier
				panCom.getLabel(labelVide).setText(e.getComponent().getName()); //on peut afficher la ligne cliquée
				operation[labelVide] = e.getComponent().getName();//on peut ajouter la ligne Ã  l'opération
			}
			if (labelVide == 5 && !( operation[0].equals( e.getComponent().getName() ) ) && !Arrays.asList(Data.LIGNES).contains(operation[3]) ) { //si la troisième ligne du calcul est différente de la ligne Ã  modifier et que la deuxime ligne choisie ne prend pas de calcul
				panCom.getLabel(labelVide).setText(e.getComponent().getName()); //on peut afficher la ligne cliquée
				operation[labelVide] = e.getComponent().getName();//on peut ajouter la ligne Ã  l'opération
			}
			...
			if (!(e.getComponent().getName().equals(operation[0]))) { //si la ligne Ã  échangée est différente de la première ligne choisie
				panCom.getLabel(labelVide).setText(e.getComponent().getName()); //on peut afficher la ligne cliquée
				operation[labelVide] = e.getComponent().getName();//on peut ajouter la ligne Ã  l'opération
			}
		}
